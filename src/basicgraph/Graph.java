...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		ArrayList<Integer> degreeSequences = new ArrayList<>();
		int numV = getNumVertices();

		for (int i = 0; i < numV; i++) {
			degreeSequences.add(getInNeighbors(i).size() + getNeighbors(i).size());
		}

		Collections.sort(degreeSequences, Comparator.reverseOrder());
...
//	 * @param The String label of the vertex
	...
	public static void main (String[] args) {
		GraphLoader.createIntersectionsFile("data/maps/ploskiy.map", "data/intersections/ploskiy.intersections");
